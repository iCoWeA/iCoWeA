...
export const getTokenDuration: (token: IdTokenResult | null) => number = (token) => {
  if (token === null) {
    return 0;
  }
...
export const isTokenExpired: (token: IdTokenResult | null) => boolean = (token) => {
  if (token === null) {
    return true;
  }
...
export const getToken: () => Promise<IdTokenResult | null> = async () => {
  try {
